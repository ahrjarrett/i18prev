...
import { BENCHMARKS, PACKAGES, PATH, RELATIVE_PATH } from './constants.js'

const serialize = (packages: readonly string[], benchmarks: readonly string[]) => {
  return [
    `export const PACKAGES = [`,
    [...packages].map((pkg) => `  '${pkg}',`).sort().join('\n'),
    [...benchmarks].map((pkg) => `  '${pkg}',`).sort().join('\n'),
    `] as const`,
...
function bump(): void {
  ...
  void fs.writeFileSync(PATH.generated_package_list, serialize(PACKAGES, BENCHMARKS))
  void PACKAGES
    .sort()
