 ...
 * @example
 *  assert.equal(
 *    bracket("@i18prev/core"),
 *    "[@i18prev/core](./packages/core)",
 *  )
 */
 ...
 * @example
 *  assert.equal(
 *    drawRelation({ name: "@i18prev/core" })("@i18prev/data"),
 *    "core(@i18prev/core) -.-> data(@i18prev/data)",
 *  )
 */
const drawRelation
...
export const drawChangelogLineItem = (pkg: { name: string, version: string }) => `${bracket(pkg.name, pkg.version)
  } - [CHANGELOG](${`https://github.com/i18prev/shared/blob/main/packages/${withoutPrefix(pkg.name)}/CHANGELOG.md`
  })`
